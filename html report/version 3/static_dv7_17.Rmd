---
title: "Proteomic consistency metrics -- Development Version with static metrics, v3"
...
QCmetrics <- rename(QCmetrics,
                    ...
                    MS2chargeALL.5andhigher = MS2Charge5andHigher_ALL,
                   )
...
# Creat dataframe for differnt charge state
...
chargestateratio_ALL <- c(sum(subset(QCmetricsLongMS2Charge_ALL, MS2ChargeState == 1,select=c(Chargeratio))), sum(subset(QCmetricsLongMS2Charge_ALL, MS2ChargeState == 2,select=c(Chargeratio))), sum(subset(QCmetricsLongMS2Charge_ALL, MS2ChargeState == 3,select=c(Chargeratio))), sum(subset(QCmetricsLongMS2Charge_ALL, MS2ChargeState == 4,select=c(Chargeratio))), sum(subset(QCmetricsLongMS2Charge_ALL, MS2ChargeState == "5andhigher",select=c(Chargeratio))))
...
QCmetrics <- rename(QCmetrics,
                    ...
                    MS2chargeID.5andhigher = MS2Charge5andHigher_ID,
                   )
...
```{r}
# generate a variable which contains the standard fond size for all plot 

fondsize <-  theme(plot.title = element_text(size = 15, face = "bold")) + 
      theme(axis.title.x = element_text(face="bold", size=14),axis.text.x= element_text(angle = 90,size=12)) + 
...
## in this case n represents the data frame contains all spectra, m represents the dataframe contains only the identified spectra and i represents the y lable for each plot , remember to put "i".
...
generate_scatter_plot <- function(kind,n,m,i)   
  ...
  if(kind == "percentile")
    ...
    g_all <- ggplot(n[n$Percentile %in% c(5,25,50,75,95),],aes(y=value, x=fractionNum, col=analyticalBasename, group=analyticalBasename)) +
      ...
      xlab("Fraction") + ylab(i) + ggtitle("For all spectra") +
      fondsize + ylim(find_min(n,m),find_max(n,m)) + theme(legend.position = "top")
      
    g_id <- ggplot(m[m$Percentile %in% c(5,25,50,75,95),], aes(y=value, x=fractionNum, col=analyticalBasename, group=analyticalBasename)) +
      facet_grid(~ Percentile) +
      geom_point() +
      scale_color_manual (values = c(plotcolor), name = "Analytical Sample") +
      xlab("Fraction") + ylab(i) + ggtitle("For identified spectra") +  
      fondsize +
...
#generate_scatter_plot("chargestate", QCmetricsLongMS2ChargeTotal_ALL, QCmetricsLongMS2ChargeTotal_ID, "MS2 Charge Ratio")

## generate a scatter plot for different percentile with separate scales
...
generate_scatter_plot_noscale <- function(kind,n,m,i)   
  ...
  if(kind == "percentile")
    ...
    g_all <- ggplot(n[n$Percentile %in% c(5,25,50,75,95),],aes(y=value, x=fractionNum, col=analyticalBasename, group=analyticalBasename)) +
      ...
      geom_point() +
      scale_color_manual (values = c(plotcolor), name = "Analytical Sample") +
      xlab("Fraction") + ylab(i) + ggtitle("For all spectra") +  fondsize
    
    g_id <- ggplot(m[m$Percentile %in% c(5,25,50,75,95),], aes(y=value, x=fractionNum, col=analyticalBasename, group=analyticalBasename)) +
      facet_grid(~ Percentile) +
      geom_point() +
      scale_color_manual (values = c(plotcolor), name = "Analytical Sample") +
      xlab("Fraction") + ylab(i) + ggtitle("For identified spectra") +  fondsize
...
## Data file 
  We used the "CompRefBI_mzml_metricInfo_all_id.07272017.tsv" data file to generate this report.
...
```{r}
### to generate the PDF report use the code 
...
### Per sample, across fraction (boxplot) , separate scales

This graph represents boxplot of the number of MS2 spectra across fraction versus the analytical sample in separate scales. Each analytical sample is represented by a different color. 
...
g_MS2box_all <- ggplot(QCmetrics, aes(y=numofMS2_ALL, x=analyticalBasename, col=analyticalBasename)) +
  ...
  scale_color_discrete(name = "Analytical sample") + fondsize +
...
# putting the all spectra and identified spectra side by side and using only one legend
if(length(analyticalSamples)<=10)
{
  grid.arrange(g_MS2box_all, g_MS2box_id, ncol=2, top= "Number of MS2 spectra across fractions")
} else {
  grid.arrange(g_MS2box_all, g_MS2box_id,ncol=2, top = "Number of MS2 spectra across fractions") +theme(legend.position="none")
}
```
<br><br>
...
This graph represents box plot of the number of MS2 spectra across fraction versus the analytical sample in the same scale. Each analytical sample is represented by a different color. 
...
### Per sample, versus fraction (scatterplot), separate scales
This graph represents scatter plot of the number of MS2 spectra versus fractions in separate scales. Each analytical sample is represented by a different color. The grey area represent the 95% confidence interval for each sample 
...
g_MS2sca_all <- ggplot(QCmetrics, aes(y=numofMS2_ALL, x=fractionNum, col=analyticalBasename)) +
  ...
  ggtitle(title_ALL) +
   fondsize + theme(axis.text.x= element_text(angle = 0)) +
...
if(length(analyticalSamples)<=10)
{
  ...
  grid_arrange_shared_legend(g_MS2sca_all, g_MS2sca_id, ncol=2, top = "Number of MS2 spectra across fractions") +theme(legend.position="none")
}
...
### Per sample, versus fraction (scatterplot), same scale
This graph represents scatter plot of the number of MS2 spectra versus fractions in the same scale. Each analytical sample is represented by a different color. The grey area represent the 95% confidence interval for each sample 
...
### Number of MS2 spectra versus fraction (boxplot), separate scale
This graph represents box plot of the number of MS2 spectra versus fractions in separate scales. Each analytical sample is represented by a different color. The grey area represent the 95% confidence interval for each sample 
...
if(length(analyticalSamples)<=10)
{
  ...
  grid_arrange_shared_legend(g_MS2box2_all, g_MS2box2_id, ncol=2, top = "Number of MS2 spectra across fractions") +theme(legend.position="none")
}
...
### Number of MS2 spectra versus fraction (boxplot), same scale
This graph represents boxplot of the number of MS2 spectra versus fractions in the same scale. Each analytical sample is represented by a different color. The grey area represent the 95% confidence interval for each sample 
...
This graph represents scatter plot of the 5%,25%,50%,75%,95% for precursor intensity values versus fractions in the same scale. Each panel is a different percentile. Each analytical sample is represented by a different color. 
...
Pmax_ALL <- log10(max(subset(QCmetricsLongPrecInt_ALL, Percentile <= 95,select=c(value))))
Pmin_ID <- log10(min(QCmetricsLongPrecInt_ID$value[QCmetricsLongPrecInt_ID$value > 0]))
Pmax_ID <- log10(max(subset(QCmetricsLongPrecInt_ID, Percentile <= 95,select=c(value))))
...
g_PIsca_all <- ggplot(QCmetricsLongPrecInt_ALL[QCmetricsLongPrecInt_ALL$Percentile %in% c(5,25,50,75,95),], 
  ...
  facet_grid(~ Percentile) +
  geom_point() +
  ylim(c(min(Pmin_ALL,Pmin_ID),max(Pmax_ALL, Pmax_ID))) +
  ggtitle(title_ALL) +
  fondsize + 
...
if(length(analyticalSamples)<=10)
{
  grid_arrange_shared_legend(g_PIsca_all, g_PIsca_id , ncol=2)
} else {
  grid_arrange_shared_legend(g_PIsca_all, g_PIsca_id, ncol=2)
}
...
### Per sample, versus fraction (scatterplot)
This graph represents scatter plot of the 5%,25%,50%,75%,95% for precursor m/z values versus fractions in the same scale. Each panel is a different percentile. Each analytical sample is represented by a different color. 
...
## Precursor molecular weight values

### Per sample, versus fraction (scatterplot)
This graph represents scatter plot of the 5%,25%,50%,75%,95% for precursor molecular weight values versus fractions in the same scale. Each panel is a different percentile. Each analytical sample is represented by a different color. 
...
This graph represents boxplot of the 5%,25%,50%,75%,95% for precursor molecular weight values across fractions in the same scale. Each panel is a different percentile. Each analytical sample is represented by a different color. 
...
g_PMWbox_all <- ggplot(QCmetricsLongPrecMW_ALL[QCmetricsLongPrecMW_ALL$Percentile %in% c(5,25,50,75,95),], 
  ...
  ggtitle("For all spectra") + fondsize +
...
g_PMWbox_id <- ggplot(QCmetricsLongPrecMW_ID[QCmetricsLongPrecMW_ID$Percentile %in% c(5,25,50,75,95),], 
  ...
  ggtitle("For identified spectra") + fondsize
...
## MS2 per MS1 values
### Per sample, versus fraction (scatterplot)
This graph represents scatterplot of the 5%,25%,50%,75%,95% for MS2 per MS1 values versus fractions in the same scale. Each panel is a different percentile. Each analytical sample is represented by a different color.
...
In this table, the meidian MS2 per MS1 value for each percentile is present, each column represents a different percentile. 
...
## Proportion of charge states for MS2 spectra
This graph represents proportion of charge states for MS2 spectra in the same scale. Each charge state of MS2 is represented by a different color.
...
g_chargeratio_all <- ggplot(Chargeratio_ALL, aes(x=chargestate, y=chargestateratio_ALL)) +
  ...
  geom_text(aes(label = round(chargestateratio_ALL,3)), vjust=-0.3, size=5) +
  scale_color_manual (values = c(plotcolor), name = "Analytical Sample") +
  scale_fill_discrete(name = "Charge State") +
  #stat_smooth(method="loess", fill="grey50", size=0)+
  ggtitle(title_ALL) +
  xlab("MS2 charge state") + ylab("To MS2 Charge2 ratio") +
...
g_chargeratio_ID <- ggplot(Chargeratio_ID, aes(y=chargestateratio_ID, x=chargestate)) +
  geom_bar(stat="identity",aes(fill = chargestate)) +
  geom_text(aes(label = round(chargestateratio_ID,3)), vjust=-0.3, size=5) +
  scale_color_manual (values = c(plotcolor), name = "Analytical Sample") +
  #stat_smooth(method="loess", fill="grey50", size=0)+
  scale_fill_discrete(name = "Charge State") +
  ggtitle(title_ID) +
...
if(length(analyticalSamples)<=10)
{
  ...
  grid_arrange_shared_legend(g_chargeratio_all, g_chargeratio_ID, ncol=2) +theme(legend.position="none")
}
